...
namespace API.SignalR
{
  public class MessageHub : Hub
  {
    ...
    public async Task SendMessage(CreateMessageDto createMessageDto)
    {
        ...
        var recipiant = await _unitOfWork.UserRepository.GetUserByUsernameAsync(createMessageDto.RecipientUsername);
        ...
        {
            var connections = await _tracker.GetConnectionsForUser(recipiant.UserName);
            if (connections != null )
            {
                await _presenceHub.Clients.Clients(connections).SendAsync("NewMessageReceived",
                    new {username = sender.UserName, knownnAs = sender.KnownAs});
            }
        }
